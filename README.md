...
$\text{BabaYaga@NLO}$ is an event generator for Bhabha scattering, $\mu^+\mu^-$,
...
- $\textrm{RECOLA}$     (https://recola.gitlab.io/)
- $\textrm{COLLIER}$    (https://collier.hepforge.org/)
- $\textrm{LOOPTOOLS}$  (https://feynarts.de/looptools/)
- $\textrm{RANLUX}$ (C version) (https://luscher.web.cern.ch/luscher/ranlux/)  

## Vacuum polarization choice

In $\textrm{BabaYaga@NLO}$, the hadronic contribution to the vacuum polarization
can be calculated by means of three different routines:
-  $\textrm{NSK}$ (default), Novosibirsk parameterization by Fedor Ignatov
   (http://cmd.inp.nsk.su/~ignatov/vpl/)

- $\textrm{HADR5N16}$, by F. Jegerlehner
   (http://www-com.physik.hu-berlin.de/~fjeger/)
- $\textrm{HMNT}$, by K. Hagiwara, A.D. Martin, D. Nomura and T. Teubner
   (see: Phys. Rev. D, 2004, 69:093003; Phys. Lett. B, 2007, 649:173).

You can choose it in the interactive menu. Please notice that
the $\textrm{HMNT}$ and $\textrm{HADR5N16}$ routines are not reliable on top of the narrow resonances,
where the only reliable is $\textrm{NSK}$.
...
- `iffpi` $\Rightarrow$ the way $F_\pi(q^2)$ is introduced in the calculation:
	- For radiative $\pi^+\pi^-\gamma$
 		- iffpi= 0 $\Rightarrow$ Pion form factor off
		- iffpi= 1 $\Rightarrow$ Pion form factor $\text{F}\times\text{sQED}$

	- For $\pi^+\pi^-$ production
		- iffpi= 0  $\Rightarrow$ Pion form factor off
		- iffpi= 1  $\Rightarrow$ Pion form factor $\text{F}\times\text{sQED}$
		- iffpi= 2 $\Rightarrow$ Pion form factor GVMD
		- iffpi= 3 $\Rightarrow$ Pion form factor FsQED
...
- `ord` $\Rightarrow$ it sets which photonic radiative corrections are included:
	- $\textrm{born}$ means Born cross section,
	- $\textrm{alpha}$ means cross section at order alpha
	- $\textrm{exp}$ is the best, exponentiated cross section
	 
- `model` $\Rightarrow$ it sets the model for radiative corrections.
	- $\textrm{matched}$ is the best one
	- $\textrm{ps}$ is very similar to older $\textrm{BabaYaga@NLO}$ releases, also in this case the ps is active only for non radiative process.
...
- $\textrm{cuts}$ (in the file cuts.f), used to impose event selection
	criteria on the generated events. Cuts are always performed
	in the lab reference frame.

- $\textrm{initstorage}$ (in the file routines.f): it initializes the environment
	to store unweighted/weighted events in the events.dat file.

- $\textrm{eventstorage}$ (in the file storage.F): it stores the
